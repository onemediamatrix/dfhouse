...
class food_wp_module3 extends WP_Widget {
     function __construct() {
	    $widget_ops = array('description' => esc_html__('Module 3: Top Posts By comments', 'food-wp'));
        parent::__construct(false, $name = ''. esc_html__('== Module 3 == Top Posts By comments', 'food-wp') .'',$widget_ops); 
    }
...
<?php if ( $title ) echo $before_title . esc_attr( $title ) . $after_title; ?>
    ...
    <ul class="articles-modules">
        <?php $food_wp_antop = new WP_Query(array('orderby' => 'comment_count', 'ignore_sticky_posts' => 1, 'posts_per_page' => esc_attr(5) )); // number to display more / less ?>
...
// register widget
function food_wp_module3_init_widget () {
    return register_widget('food_wp_module3');
  }
add_action ('widgets_init', 'food_wp_module3_init_widget');
?>